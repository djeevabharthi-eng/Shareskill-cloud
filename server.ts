...
let currentUser: UserProfile = users[0]; // Jeeva bharathi D
...
try {
  if (fs.existsSync(USERS_FILE)) {
    ...
    if (Array.isArray(parsedUsers) && parsedUsers.length > 0) {
      ...
      if (foundCurrent) currentUser = foundCurrent;
    }
  } else {
...
async function startServer() {
  ...
  app.post("/api/auth/login", (req, res) => {
    ...
    if (!found) {
      // Return Jeeva by default or create authenticated user
      found = currentUser;
    }
    currentUser = found;
    res.json(currentUser);
  ...
  app.post("/api/auth/signup", (req, res) => {
    ...
    const newUser: UserProfile = {
      ...
      email: email || "member@shareskill.cloud",
      city: "Thanjavur",
      country: "India",
      bio: "",
      teachSkills: [],
